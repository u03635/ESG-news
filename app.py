...
def search_latest_news_with_sources(query, max_results=10, timelimit=None):
    """即時搜尋，將【網址】也一併打包給 AI，讓 AI 能夠針對各別項目進行精準引用"""
    try:
        ...
        for r in results:
            ...
            if title and href:
                # 關鍵修改：把 href 網址也變成 context 的一部分，讓 AI 知道每一則的連結
                context += f"【標題】: {title}\n【網址】: {href}\n【內容摘要】: {body}\n\n"
...
def topic():
    ...
    if topic_name == "即時法規與新制快報":
        ...
        search_context = search_latest_news_with_sources(search_query, max_results=20, timelimit='y')
        ...
        5. 【最重要】：請務必在「每一則」整理資訊的最後面，利用 Markdown 語法附上對應的資料來源超連結，格式為：`[👉 點此查看資料來源]({{'網址'}})`。
        ...
        try:
            ...
            # 現在 AI 會自己排版連結，所以我們直接回傳 AI 的文字即可
...
def analyze_diff():
    ...
    3. 【重要】：請務必在提及特定法規或新聞段落的下方，附上對應的參考來源，格式為：`[👉 參考來源]({{'網址'}})`。
...
def query():
    ...
    【要求】：請在回答中適當引用資料，並在段落後方使用 Markdown 格式附上超連結：`[參考來源]({{'網址'}})`。
